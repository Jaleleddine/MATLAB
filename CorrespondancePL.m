...
% Algorithme 3: Programmation Linéaire:
% Algorithm: Linear Programming Approach: 
% (input: AG, AH; output: P, J(P))                                            
%**************************************************************************
% 1: Initialisation: AG et AH sont deux matrices carrées:
% AG=input('Matrix AG :  ')
% AH=input('Matrix AH :  ')
[n1,m1]=size(AG);
[n2,m2]=size(AH);
...
B % à afficher
%**************************************************************************
% 3: Résoudre le nouveau problème linéaire obtenu (Simplexe) :
% min p,S,T SOMME (Sm+Tm) ,avec m=1..N
% Sous contraintes
% AGH.p+S–T=0
% B.p=e 
% P>=0,S>=0,T>=0
% Où Sm, et Tm deux vecteurs de taille (N, 1)
...
x = linprog(c,[ ],[ ],A,b,zeros(size(c)),[ ]) % à afficher
v=zeros(1,N);
for i=1:N 
v(i)=x(i);
end
...
P % à afficher
%**************************************************************************
%4: Calculer la différence J(P) en utilisant l’expression:
%J(P) = ||P.AG.(P)T–AH||^2
